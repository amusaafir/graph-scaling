...
void Topology::add_edge_interconnection_between_graphs(Sampled_Graph_Version* graph_a, Sampled_Graph_Version* graph_b, std::vector<Bridge_Edge>& bridge_edges) {
	for (int i = 0; i < _AMOUNT_INTERCONNECTIONS; i++) {
		...
		sprintf(bridge_edge.source, "%c%c%d", graph_a->label_1, graph_b->label_2, vertex_a);
		sprintf(bridge_edge.destination, "%c%c%d", graph_b->label_1, graph_b->label_2, vertex_b);
		...
		if (_FORCE_UNDIRECTED_BRIDGES) {
			...
			sprintf(bridge_edge_undirected.source, "%c%c%d", graph_b->label_1, graph_b->label_2, vertex_b);
			sprintf(bridge_edge_undirected.destination, "%c%c%d", graph_a->label_1, graph_b->label_2, vertex_a);

...
void Sampling::sample_graph(char* input_path, char* output_path) {
	...
	gpuErrchk(cudaMemcpy(d_sampled_vertices, sampled_vertices->vertices, sizeof(int) * _graph_io->SIZE_VERTICES, cudaMemcpyHostToDevice));
	...
	Edge* d_edge_data;
	gpuErrchk(cudaMalloc((void**)&d_edge_data, sizeof(Edge) * _graph_io->SIZE_EDGES));
	...
	if (h_edge_count >= _graph_io->SIZE_EDGES + 1) {
	...
	_graph_io->write_output_to_file(results, output_path);
	cudaFree(d_offsets);
	cudaFree(d_indices);
...
Sampled_Vertices* Sampling::perform_edge_based_node_sampling_step(int* source_vertices, int* target_vertices) {
	...
	sampled_vertices->vertices = (int*)calloc(_graph_io->SIZE_VERTICES, sizeof(int));
	int collected_amount = 0;
	
	while (collected_amount < amount_total_sampled_vertices) {
		// Pick a random vertex u
		std::uniform_int_distribution<int> range_edges(0, (_graph_io->SIZE_EDGES - 1)); // Don't select the last element in the offset
